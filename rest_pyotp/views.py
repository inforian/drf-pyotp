...
class V3PyotpViewset(viewsets.GenericViewSet):
    """Pyotp Viewset, for v3

    """
    # Fixme: This is wrong requirement or fix, we have to find out other way asap
    queryset = models.PyOTP.objects.all()
    ...
    def verify(self, request, ):
        ...
        valid_otp = serializer.verify_otp(serializer.data.get('otp'), serializer.data.get('unique_identifier'))
        if not valid_otp:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        return Response(status=status.HTTP_200_OK)
